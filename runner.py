...
def main():
    ...
    if not args:
        ...
        print()
        print(f"Message：{result.message}")
    elif args[0] == "no-tool-use-patch-apply-dry-run":
        ...
        result = run_no_tool_use_controlled_patch_apply_sample_dry_run(sample=sample_type)

        print()
        print(f"EXECUTION_MODE=no_tool_use_controlled_patch_apply_dry_run")
        print(f"PATCH_APPLY_SAMPLE={sample_type}")
        print(f"PARSE_STATUS={result.parse_status}")
        print(f"PARSE_CHECK_RESULT={result.parse_check_result}")
        print(f"VALIDATION_STATUS={result.validation_status}")
        ...
        print(f"EMPTY_PATCH={'yes' if result.empty_patch else 'no'}")
        print(f"ALLOWED_SCOPE_PASS={'yes' if result.allowed_scope_pass else 'no'}")
        print(f"FORBIDDEN_SCOPE_PASS={'yes' if result.forbidden_scope_pass else 'no'}")
        print(f"PATCH_PARSE_PASS={'yes' if result.patch_parse_pass else 'no'}")
        print(f"PATCH_FILE_CONSISTENCY_PASS={'yes' if result.patch_file_consistency_pass else 'no'}")
        print(f"PATCH_APPLY_BLOCKED={'yes' if result.patch_apply_blocked else 'no'}")
        print(f"COMMAND_EXECUTION_BLOCKED={'yes' if result.command_execution_blocked else 'no'}")
        ...
        print(f"HUMAN_REVIEW_REQUIRED={result.human_review_required}")
        print(f"READY_FOR_FUTURE_CONTROLLED_APPLY={'yes' if result.ready_for_future_controlled_apply else 'no'}")
        violations_str = "; ".join(result.violations) if result.violations else "NONE"
        print(f"VIOLATIONS={violations_str}")
        print(f"CHECK_RESULT={result.check_result}")
        print()
        ...
        print()
        print(f"MESSAGE：{result.message}")
    elif args[0] == "stage8-continuous-dry-run":
        # T144: Stage 8 continuous runner dry-run planner
        sample_type = None
        max_tasks_val = STAGE8_MAX_TASKS_DEFAULT if 'STAGE8_MAX_TASKS_DEFAULT' in dir() else 1
        i = 1
        while i < len(args):
            if args[i] == "--sample" and i + 1 < len(args):
                sample_type = args[i + 1]
        ...
        result = run_stage8_continuous_runner_dry_run_planner(
        ...
        print(f"CHECKPOINT_REQUIRED={result.checkpoint_required}")
        print(f"APPROVAL_RECORD_REQUIRED={result.approval_record_required}")
        print(f"GIT_BACKUP_REQUIRED={result.git_backup_required}")
        ...
        print()
        print(f"MESSAGE：{result.message}")
    elif args[0] == "stage8-real-controlled-execution-dry-run":
        ...
        result = run_stage8_real_controlled_execution_dry_run(
            project_root=str(PROJECT_ROOT),
            max_tasks=max_tasks_val,
            sample=sample_type,
        )
        ...
        print(f"WORKSPACE_STATUS_BEFORE={result.workspace_status_before}")
        print(f"WORKSPACE_STATUS_AFTER={result.workspace_status_after}")
        print(f"STAGED_FILES_BEFORE={result.staged_files_before}")
        print(f"CURRENT_BRANCH={result.current_branch}")
        print(f"LAST_COMMIT_BEFORE={result.last_commit_before}")
        print(f"STAGE_BOUNDARY_CHECK={result.stage_boundary_check}")
        print(f"SAFETY_GATE_PASSED={result.safety_gate_passed}")
        print(f"SAFETY_GATE_FAILED={result.safety_gate_failed}")
        print(f"EXECUTION_GATE_PASSED={result.execution_gate_passed}")
        print(f"EXECUTION_GATE_FAILED={result.execution_gate_failed}")
        print(f"GATE_CHECKS_PASSED={result.gate_checks_passed}")
        print(f"GATE_CHECKS_FAILED={result.gate_checks_failed}")
        print(f"FAILED_CHECKS={result.failed_checks}")
        print(f"ALLOWED={result.allowed}")
        ...
        print(f"COMMAND_ALLOWLIST={result.command_allowlist}")
        print(f"PUSH_ALLOWED={result.push_allowed}")
        print(f"REAL_EXECUTION_ALLOWED={result.real_execution_allowed}")
        print(f"RESUME_ALLOWED={result.resume_allowed}")
        print(f"REWORK_REQUIRED={result.rework_required}")
        ...
        print(f"STAGE8_EXECUTION_STARTED={result.stage8_execution_started}")
        print(f"REAL_CONTINUOUS_EXECUTION_STARTED={result.real_continuous_execution_started}")
        print(f"CONTINUOUS_AUTO_ADVANCE_USED={result.continuous_auto_advance_used}")
        print(f"REAL_GIT_ADD_USED={result.real_git_add_used}")
        print(f"REAL_GIT_COMMIT_USED={result.real_git_commit_used}")
        print(f"REAL_GIT_PUSH_USED={result.real_git_push_used}")
        ...
        print()
        print(f"MESSAGE：{result.message}")
    elif args[0] == "stage8-real-controlled-single-step-trial":
        # T152: Stage 8 max_tasks=1 real controlled single-step execution trial
        sample_type = None
        max_tasks_val = 1
        i = 1
        while i < len(args):
        ...
        result = run_stage8_real_controlled_single_step_execution_trial(
            project_root=str(PROJECT_ROOT),
            max_tasks=max_tasks_val,
            sample=sample_type,
        )
        ...
        print(f"STAGE={result.stage}")
        print(f"MODE={result.mode}")
        print(f"TRIAL_MODE={result.trial_mode}")
        print(f"MAX_TASKS={result.max_tasks}")
        print(f"MAX_TASKS_POLICY={result.max_tasks_policy}")
        print(f"TASKS_ATTEMPTED={result.tasks_attempted}")
        print(f"TASKS_COMPLETED={result.tasks_completed}")
        print(f"CURRENT_TASK={result.current_task}")
        print(f"NEXT_PENDING_TASK={result.next_pending_task}")
        ...
        print(f"FAILURE_REASONS={result.failure_reasons}")
        print(f"REQUIRED_ACTIONS={result.required_actions}")
        print(f"TRIAL_ALLOWED={result.trial_allowed}")
        print(f"TRIAL_DECISION={result.trial_decision}")
        print(f"NEXT_TASK_EXECUTED={result.next_task_executed}")
        print(f"BUSINESS_CODE_MODIFIED={result.business_code_modified}")
        print(f"ALLOWED_SCOPE={result.allowed_scope}")
        print(f"PLANNED_FILES={result.planned_files}")
        print(f"COMMAND_ALLOWLIST={result.command_allowlist}")
        print(f"PUSH_ALLOWED={result.push_allowed}")
        ...
        print(f"APPROVAL_RECORD_PATH={result.approval_record_path}")
        print(f"CHECKPOINT_PATH={result.checkpoint_path}")
        print(f"TRIAL_REPORT_PATH={result.trial_report_path}")
        print(f"STAGE8_EXECUTION_STARTED={result.stage8_execution_started}")
        print(f"REAL_CONTINUOUS_EXECUTION_STARTED={result.real_continuous_execution_started}")
        print(f"CONTINUOUS_AUTO_ADVANCE_USED={result.continuous_auto_advance_used}")
        print(f"REAL_GIT_ADD_USED={result.real_git_add_used}")
